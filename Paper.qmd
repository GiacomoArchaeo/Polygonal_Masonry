...
**Data and code:** Data and code are available at https://doi.org/10.5281/zenodo.7555248

**Color should be used for figure 2 and for figures 9 to 13**
...
tA_volstones <- tA_facadevol*2*sectionheight
...
he building process of the three hillforts may be decomposed into three main stages: 1. Quarrying and material extraction, 2. Site preparation, 3. On-site assembly and shaping. For this study, we assume minimal transportation costs. AHS identifies quarry sites directly on the hilltops of fortified Samnite settlements, often in close proximity to wall circuits. In some cases, bedrock was quarried to provide level foundations for walls (e.g. Renda 2017, 2018). This suggests a commonly seen arrangement in which stone for polygonal masonry was extracted in loco or from sources close to ongoing construction (Brown 1951: 35; De Rossi 2009).
...
# Energetics index m^3/ph
in_buildstoneupto0.2_3m <- 0.037
in_buildstone0.2to0.5_3m <- 0.025
in_buildstoneover0.5_3m <- 0.020

in_buildstoneupto0.2_5m <- 0.027
in_buildstone0.2to0.5_5m <- 0.019
in_buildstoneover0.5_5m <- 0.015

# Summary
energetics <- matrix(1:6, nrow = 3, ncol = 2)
energetics <- rbind(c(in_buildstoneupto0.2_3m,in_buildstoneupto0.2_5m),
                    c(in_buildstone0.2to0.5_3m,in_buildstone0.2to0.5_5m),
                    c(in_buildstoneover0.5_3m,in_buildstoneover0.5_5m))
rownames(energetics) <- c("Assembly up to 0.2 m^3 (m^3/ph)",
                       "Assembly between 0.2 and 0.5 m^3 (m^3/ph)",
                       "Assembly above 0.5 m^3 (m^3/ph)")
colnames(energetics) <- c("Rates (3 m)", "Rates (5 m)")
kable(energetics, digits = 4) %>%
  kable_styling(bootstrap_options = c("condensed"),
                full_width = T,
                position = "left")
...
# 3 m
walldepth_3m <- sectiondepth
wallheight_3m <- 3

## Material acquisition
### Type-A
tA_materialstone_3m <- (tA_volstones+tA_volstones*in_quarrylost)*wallheight_3m/in_quarrywall
tA_materialrubble_3m <- tA_volrubble*wallheight_3m/in_quarryrubble
tA_materialtotal_3m <- tA_materialstone_3m+tA_materialrubble_3m

### Type-B
tB_materialstone_3m <- (tB_volstones+tB_volstones*in_quarrylost)*wallheight_3m/in_quarrywall
tB_materialrubble_3m <- tB_volrubble*wallheight_3m/in_quarryrubble
tB_materialtotal_3m <- tB_materialstone_3m+tB_materialrubble_3m

## Leveling the terrain
### Type-A
tA_leveling_3m <- walldepth_3m/in_leveling
### Type-B
tB_leveling_3m <- (walldepth_3m/2)/in_leveling

## Wall assembly
### Type-A
tA_assemblyouter_3m <- (((tA_facadevol*(composition["upto0.2","tA"]/100))/in_buildstoneupto0.2_3m)+
  ((tA_facadevol*(composition["0.2to0.5","tA"]/100))/in_buildstone0.2to0.5_3m)+
  ((tA_facadevol*(composition["over0.5","tA"]/100))/in_buildstoneover0.5_3m))*wallheight_3m
tA_assemblyinner_3m <- (tA_facadevol/in_buildstoneupto0.2_3m)*wallheight_3m
tA_assemblyfill_3m <- tA_volrubble*wallheight_3m/in_buildrubbleearth
tA_assemplytotal_3m <- tA_assemblyouter_3m+tA_assemblyinner_3m+tA_assemblyfill_3m
### Type-B
tB_assemblyouter_3m <- (((tB_facadevol*(composition["upto0.2","tB"]/100))/in_buildstoneupto0.2_3m)+
  ((tB_facadevol*(composition["0.2to0.5","tB"]/100))/in_buildstone0.2to0.5_3m)+
  ((tB_facadevol*(composition["over0.5","tB"]/100))/in_buildstoneover0.5_3m))*wallheight_3m
tB_assemblyfill_3m <- tB_volrubble*wallheight_3m/in_buildrubbleearth
tB_assemplytotal_3m <- tB_assemblyouter_3m+tB_assemblyfill_3m

## Supervision
### Type-A
tA_sup_3m <- in_supervision*(tA_materialtotal_3m+tA_leveling_3m+tA_assemplytotal_3m)
### Type-B
tB_sup_3m <- in_supervision*(tB_materialtotal_3m+tB_leveling_3m+tB_assemplytotal_3m)

## Total
### Type-A
tA_total_3m <- tA_materialtotal_3m+tA_leveling_3m+tA_assemplytotal_3m+tA_sup_3m
### Type-B
tB_total_3m <- tB_materialtotal_3m+tB_leveling_3m+tB_assemplytotal_3m+tB_sup_3m


# 5 m
walldepth_5m <- sectiondepth
wallheight_5m <- 5

## Material acquisition
### Type-A
tA_materialstone_5m <- (tA_volstones+tA_volstones*in_quarrylost)*wallheight_5m/in_quarrywall
tA_materialrubble_5m <- tA_volrubble*wallheight_5m/in_quarryrubble
tA_materialtotal_5m <- tA_materialstone_5m+tA_materialrubble_5m

### Type-B
tB_materialstone_5m <- (tB_volstones+tB_volstones*in_quarrylost)*wallheight_5m/in_quarrywall
tB_materialrubble_5m <- tB_volrubble*wallheight_5m/in_quarryrubble
tB_materialtotal_5m <- tB_materialstone_5m+tB_materialrubble_5m

## Leveling the terrain
### Type-A
tA_leveling_5m <- walldepth_5m/in_leveling
### Type-B
tB_leveling_5m <- (walldepth_5m/2)/in_leveling

## Wall assembly
### Type-A
tA_assemblyouter_5m <- (((tA_facadevol*(composition["upto0.2","tA"]/100))/in_buildstoneupto0.2_5m)+
  ((tA_facadevol*(composition["0.2to0.5","tA"]/100))/in_buildstone0.2to0.5_5m)+
  ((tA_facadevol*(composition["over0.5","tA"]/100))/in_buildstoneover0.5_5m))*wallheight_5m
tA_assemblyinner_5m <- (tA_facadevol/in_buildstoneupto0.2_5m)*wallheight_5m
tA_assemblyfill_5m <- tA_volrubble*wallheight_5m/in_buildrubbleearth
tA_assemplytotal_5m <- tA_assemblyouter_5m+tA_assemblyinner_5m+tA_assemblyfill_5m
### Type-B
tB_assemblyouter_5m <- (((tB_facadevol*(composition["upto0.2","tB"]/100))/in_buildstoneupto0.2_5m)+
  ((tB_facadevol*(composition["0.2to0.5","tB"]/100))/in_buildstone0.2to0.5_5m)+
  ((tB_facadevol*(composition["over0.5","tB"]/100))/in_buildstoneover0.5_5m))*wallheight_5m
tB_assemblyfill_5m <- tB_volrubble*wallheight_5m/in_buildrubbleearth
tB_assemplytotal_5m <- tB_assemblyouter_5m+tB_assemblyfill_5m

## Supervision
### Type-A
tA_sup_5m <- in_supervision*(tA_materialtotal_5m+tA_leveling_5m+tA_assemplytotal_5m)
### Type-B
tB_sup_5m <- in_supervision*(tB_materialtotal_5m+tB_leveling_5m+tB_assemplytotal_5m)

## Total
### Type-A
tA_total_5m <- tA_materialtotal_5m+tA_leveling_5m+tA_assemplytotal_5m+tA_sup_5m
### Type-B
tB_total_5m <- tB_materialtotal_5m+tB_leveling_5m+tB_assemplytotal_5m+tB_sup_5m


# Energetic matrix
estimates <- matrix(1:32, nrow = 8, ncol = 4)
estimates <- rbind(c(tA_materialstone_3m,tB_materialstone_3m,tA_materialstone_5m,tB_materialstone_5m),
                 c(tA_materialrubble_3m,tB_materialrubble_3m,tA_materialrubble_5m,tB_materialrubble_5m),
                 c(tA_leveling_3m,tB_leveling_3m,tA_leveling_5m,tB_leveling_5m),
                 c(tA_assemblyouter_3m,tB_assemblyouter_3m,tA_assemblyouter_5m,tB_assemblyouter_5m),
                 c(tA_assemblyinner_3m,0,tA_assemblyinner_5m,0),
                 c(tA_assemblyfill_3m,tB_assemblyfill_3m,tA_assemblyfill_5m,tB_assemblyfill_5m),
                 c(tA_sup_3m,tB_sup_3m,tA_sup_5m,tB_sup_5m),
                 c(tA_total_3m,tB_total_3m,tA_total_5m,tB_total_5m))
rownames(estimates) <- c("Stone quarry (ph/m)","Rubble quarry (ph/m)","Leveling (ph/m)","Assembly outer façade (ph/m)","Assembly inner façade (ph/m)","Assembly fill (ph/m)","Supervision (ph/m)","Total wall construction (ph/m)")
...
Using the methods developed here, we created an energetics model taking into account each site’s characteristics. Digital documentation and statistical characterisation supported volumetric reconstructions adapted to different styles of polygonal masonry and guided selection of appropriate comparative data. Results provide a linear cost for generalised structures of both Type A and B hillforts in Samnium. We note the impact on production times implied by a change in wall-height. As observed, Samnite hillforts appear lower than contemporary urban fortifications. Nonetheless, in cases where walls reached 5 m, something only exhibited in exceptional cases, costs were significantly amplified. As we see, the change of height from 3 to 5 m implies an almost twofold increase of costs, as greater energy is required to assemble taller structures. Another major factor was fortification-type. Expectedly, two-curtain Type A walls show 35% higher linear costs than one-curtain Type B walls, 161 to 121 ph/m. Applying these linear rates provides a sense of the overall labor required to build our four hillfort sites. Results distribute according to each site’s scale but raise interesting questions when returning to functions assigned by the literature to each hillfort. Substantial investment of labor was required to make the large, complex fortifications of Monte Cila (K73), of which two-fifth of the enclosed area is very steep and unlikely to have hosted inhabitation or agriculture, while the whole site shows very few traces of settlement. For the fortifications of Monte Saraceno di Cercemaggiore (K110) or Monte Santa Croce di Piana di Monte Verna (K105), preliminary archaeological data suggest possible permanent or semi-permanent occupation at least in part of the enclosed area. However, these sites were comparatively less costly, totalling around less than half of the overall investment of Monte Cila. The small, narrow circuit of Montefalcone (K35) does not seem appropriate for substantial occupation. The labor investment required remains notable, representing perhaps half year's labor for a team of 100 workers. This investment may speak to purposes beyond supporting local pastoral activities in the region. The site’s dominant view of the Adriatic Sea together with its steep position above main routes suggests the importance of visibility. The energy investment required by its creation may thus reflect an attempt to display power on the part of its builder.

Our results may now be expanded to broader study of Samnite architecture within the framework of the ongoing AHS in combination with novel field data and quantitative approaches (Fontana 2022). The approach here also intends to support cross-cultural or cross-temporal comparison with the labor costs of similar masonry structures in the Mediterranean and beyond. Our study hopes to provide a first step in the inclusion of Samnite sites, and other non-Roman or non-urban sites of polygonal masonry, into wider assessments of architecture’s historical impacts in the ancient world.
